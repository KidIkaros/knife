...
pub static KERNEL_CATALOG: &[SinkDef] = &[
    ...
    nt!("IoSetCompletionRoutineEx", "callback", "IRP completion routine with unload-safe registration", 2),
    nt!("IoQueueWorkItem", "callback", "queue a driver work-item callback", 2),
    nt!("KeInitializeDpc", "callback", "initialize a deferred procedure callback", 2),
    nt!("KeInsertQueueDpc", "callback-activation", "queue an initialized DPC object for deferred execution", 2),
    nt!("KeSetTimer", "callback-activation", "arm a timer with an optional initialized DPC object", 2),
    nt!("KeSetTimerEx", "callback-activation", "arm a periodic timer with an optional initialized DPC object", 2),
    nt!("KeRemoveQueueDpc", "callback-cancellation", "attempt to remove a queued DPC before execution", 2),
    nt!("KeCancelTimer", "callback-cancellation", "attempt to cancel a timer before expiration", 2),
